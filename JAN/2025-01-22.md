...
|  | 정의 | 비고 |
| --- | --- | --- |
| (선택) 인터페이스 | 클래스들이 구현해야 하는 동작을 지정하는데 사용되는 추상 자료형 | 같은 인터페이스를 구현한 클래스들이 서로 다른 역할을 하면 안 된다.
그 이유는 여러 가지가 있겠지만, 크게 SOLID 원칙 중 프로그램의 객체는 프로그램의 정확성을 깨뜨리지 않으면서 하위 타입의 인스턴스로 바꿀 수 있어야 한다는 의미의 LSP (리스코프 치환 원칙)에 위배되기 때문이다.
ex) 자동차 인터페이스의 엑셀은 앞으로 가라는 기능, 뒤로 가게 구현하면 → LSP 위반!
인터페이스를 구현한 구현체는 믿고 사용하려면, 이 원칙이 필요하다. |
| (선택) POJO | Plain Old Java Object의 약어로, 오래된 방식의 간단한 자바 객체, 즉 자바로 생성하는 순수한 객체를 뜻한다. | 예를 들어 어떤 클래스나 인터페이스를 상속받거나 구현하지 않은 클래스는 POJO 프로그래밍이다.
자바에서 만든 표준 인터페이스를 구현했어도 POJO 프로그래밍이라고 할 수 있다.
다른 기술에 의존하지 않으면서 재사용이 가능해지기 때문에 지향하는 것이 좋다. |
| 생성자 | 객체의 초기화하기 위해 사용하는 메서드 | 객체가 처음 생성될 때 호출되어 필요한 속성들을 초기화한다. |
| this() | 같은 클래스의 다른 생성자를 호출할 때 사용하는 메서드 | this는 인스턴스 자신을 가리키는 참조변수이다. |
| super() | 자식 생성자 안에서 부모 클래스의 생성자를 호출할 때 사용하는 메서드 | super는 부모 클래스를 가리키는 참조변수이다.
주로 부모, 자식 클래스의 변수나 메서드의 이름이 같을 때 사용한다. |
| Method Overloading | 같은 클래스 내에서 메서드 이름은 동일하지만, 매개변수가 다른 메서드를 정의하는 것 | 같은 이름을 가진 메서드들이 다양한 역할을 수행할 수 있으며, 리턴타입도 달라질 수 있다.
주로 메서드의 다형성을 제공하기 위해 사용한다. |
| Method Overriding | 상속 관계에 있는 부모 클래스의 메서드를 재정의하는 것 | 주로 상속받은 메서드를 자식 클래스의 요구에 맞게 변경하기 위해 사용한다. |
| static | 프로그램의 컴파일 시간에 메모리를 할당하는 것 | static을 통해 선언된 정적 변수는 프로그램의 생명 주기와 동일한 생명 주기를 갖는다.
Stack 영역 또는 Heap 영역에 할당되지 않고, Method 영역에 할당된다는 특징이 있다. |
| 상속 | 클래스의 기능을 확장하거나 재정의하기 위해 기존의 클래스를 재활용하여 새로운 클래스를 작성하는 것 | 부모님으로부터 자산을 상속받는 개념의 상속과는 다른 개념이며, extends 예약어를 사용한다. |
| (선택) 다중상속 | 어떤 클래스가 하나 이상의 상위 클래스로부터 여러 가지 행동이나 특징을 상속받을 수 있는 것 | 자바는 클래스의 다중 상속이 불가능하다.
인터페이스를 사용해 비슷한 기능을 구현할 수는 있지만, 인터페이스는 상속받는 것이 아니라 구현하는 것이기 때문에 인터페이스의 다중 상속이라는 표현은 잘못된 표현이다. |
| (선택) 다이아몬드 문제 | 다중 상속을 지원하는 언어에서 여러 경로로 상속된 공통 부모의 메서드나 속성을 처리할 때 발생하는 충돌 문제 |    A
 /   \
B   C
 \   /
   D
`B`와 `C`가 `A`를 상속받고, `D`가 `B`와 `C`를 상속받는 구조
`D`에서 `A`의 메서드를 호출하려고 하면 `B`와 `C`를 통해 각각 상속된 `A`의 메서드 중 어떤 것을 사용해야 하는지에 대한 규칙이 필요하다. |
